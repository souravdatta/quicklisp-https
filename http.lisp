...
(defvar *fetch-scheme-functions*
  '(("http" . http-fetch-with-wget))
  "assoc list to decide which scheme-function are called by FETCH function.")

(defun fetch (url file &rest rest) ;; Look here Sourav
...
(defun url-scheme (url)
  (cond
    ...
    (t (if (string= (subseq url 0 5) 'https)
           'https
           'http))))

(defun http-fetch-with-wget (url file &key (follow-redirects t) quietly
                                           (if-exists :rename-and-delete)
                                           (maximum-redirects 0))
  (format t "WARNING: Using system WGET instead of native Lisp code~%")
  (format t "INFO: URL ~A~%" url)
  (when (equal (url-scheme url) 'http)
    (format t "WARNING: Switching to HTTPS~%")
    (setf url (concatenate 'string "https" (subseq url 4))))
  (format t "INFO: New URL ~A~%" url)
  (let ((command (format nil "/usr/bin/wget ~A -O ~A" url file))) ;; Note to programmer: change path to your WGET or CURL or whatever you are using
